var PhaserGameObject = (function () {
    ...
    PhaserGameObject.prototype.init = function (el, parent, options) {
        ...
        function preloadComplete() {
            var game = phaserMaster.game();
            var padding = 15;
            var header1 = phaserTexts.add({ name: 'header', font: 'gem', size: 18, default: 'Sprites and tweening!' });
            phaserTexts.alignToTopCenter('header', 10);
            phaserGroup.layer(10).add(header1);
            var instructions = phaserTexts.add({ name: 'instructions', size: 14, font: 'gem', default: "Press [A] to add sprites" });
            ...
            var spriteCount = phaserTexts.add({ name: 'spriteCount', size: 14, font: 'gem', default: "" });
        ...
        function update() {
            ...
            if (phaserControls.checkWithDelay({ isActive: true, key: 'A', delay: 100 })) {
                var i = phaserSprites.getAll('ARRAY').length;
                var newSprite = phaserSprites.add({ name: "star" + i, group: 'g1', reference: 'star', x: game.world.randomX, y: game.world.randomY + game.canvas.height });
                newSprite.alpha = 1;
